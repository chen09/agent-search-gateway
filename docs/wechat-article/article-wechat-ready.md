...
[GitHub - chen09/agent-search-gateway](https://github.com/chen09/agent-search-gateway)
...
reranker 支持 sentence-transformers CrossEncoder，但默认关闭。原因很现实：它会拉 PyTorch。我的机器是 Mac mini，基础服务不应该一上来就强依赖 PyTorch。
...
Cursor 的 User Rules GitHub/GitLab import 里，填仓库 URL：
...
整个过程中真正重要的不是“无脑让 AI 写”，而是持续做工程判断：

- 不做 Tavily clone，做 gateway
- Brave 不默认启用
- PyTorch 不进基础依赖
- provider key 不给 agent
- API host 用 `api.agentsearchgateway.com`
- root domain 留给人类页面
- Cursor User Rule 不放 `.cursor/rules` 作为对外入口
- Docker image 要公开发布
- README 和 Docker Hub 都要中日英三语
- MCP 要能用 `uvx` 从 GitHub 直接启动
- Cursor Rules 要能从 GitHub import，并且真的触发工具调用
...
- [Firecrawl Billing](https://docs.firecrawl.dev/pt-BR/billing)
